/**
 * src/pages/user/CustomTakeTestPage.jsx
 *
 * Test-taking page for standalone custom category tests.
 * Route: /test/custom/:testId/take
 *
 * Visual/interaction design mirrors the premium default TakeTestPage
 ...
 * tab-switch toast) — but a custom test is always a SINGLE flat list of
 * questions. There are no sections here; that concept belongs only to
 * default-category tests and is intentionally not reproduced.
 *
 * Data contract (unchanged from the previous implementation):
 *   - Fetches MCQs from GET /api/custom-tests/:testId/mcqs (userProtect)
 *   - Submits to POST /api/results/submit with sectionType: "standalone"
 *   - After submission, navigates to /result/custom (CustomTestResultPage)
...
// ── Main Page Component ───────────────────────────────────────
export default function CustomTakeTestPage() {
  ...
  const timerKeyRef = useRef(`timer_custom_${testId}`);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside className="w-74 border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 flex flex-col justify-between hidden lg:flex shrink-0">
          <div className="space-y-5">
